...
/**
* A camera that follows a spatial and can turn around it by dragging the mouse and that collides with the physics space.
* @author shirkit.
* @author neon for the {@link ChaseCamera} code.
* @author unkown for the base code for the physics collision.
*/
...
protected float maxVerticalRotation = 89f * FastMath.DEG_TO_RAD;
...
protected Vector3f maxPos = new Vector3f();
protected float maxDistanceByZoom = 50f;
...
* @param physicsSpace the physics space where this camera will colide with.
*/

    public PhysicsChaseCamera(Camera cam, final Spatial target, InputManager inputManager, PhysicsSpace physicsSpace) {
        this.cam = cam;
        this.target = target;
    ...
    private void computePosition() {
        ...
        hDistance = (distance) * FastMath.sin((FastMath.PI / 2) - vRotation);
        ...
        maxPos.set(hDistance * FastMath.cos(rotation), (maxDistanceByZoom) * FastMath.sin(vRotation), hDistance * FastMath.sin(rotation));

        maxPos.addLocal(target.getWorldTranslation());
...
//nothing to render
     ...
     * will rotates around the target when draging with the mouse
     *
     * default is 5
     *
     ...
     * camera to roughtly stop when the target stops moving
     *
     ...
     * will go in the target trail when it moves.
     *
     * default is 0.5;
     *
    ...
    LinkedList<PhysicsRayTestResult> testResults = new LinkedList<PhysicsRayTestResult>();

    public void collide() {
        hitFraction = 1f;
        testResults.clear();
        physicsSpace.rayTest(target.getWorldTranslation(), maxPos, testResults);
